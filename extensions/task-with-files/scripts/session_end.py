#!/usr/bin/env python3
"""SessionEnd Hook - checks task completion status on session end."""
    ...
    try:
        ...
        if check_task_completed(task_path):
            ...
            print(
                json.dumps(
                    {
                        "systemMessage": f"[task-with-files] Task '{task_name}' completed and reset."
                    }
                ),
                flush=True,
            )
            ...
            print(
                json.dumps(
                    {
                        "systemMessage": f"[task-with-files] Session ended. Incomplete task '{task_name}' saved. Run /task:resume to continue."
                    }
                ),
                flush=True,
            )
